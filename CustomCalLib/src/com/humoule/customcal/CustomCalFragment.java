...
import com.humoule.customcal.adapter.InfinitePagerAdapter;
import com.humoule.customcal.adapter.MonthPagerAdapter;
import com.humoule.customcal.adapter.WeekdayArrayAdapter;
import com.humoule.customcal.callback.CustomCalListener;
import com.humoule.customcal.util.CalendarHelper;
import com.humoule.customcal.view.InfiniteViewPager;
...
public class CustomCalFragment extends DialogFragment {
	public String TAG = "CustomCalFragment";
	...
	public final static String FIVE_WEEKS_IN_CALENDAR = "fiveWeeksInCalendar";
	...
	/**
	 * customCalData belongs to Custom Calendar View
	 */
	protected HashMap<String, Object> customCalData = new HashMap<String, Object>();
	 ...
	 */
	private boolean fiveWeeksInCalendar = true;

	/**
	 * datePagerAdapters hold 4 adapters, meant to be reused
	 */
	protected ArrayList<CustomCalGridAdapter> datePagerAdapters = new ArrayList<CustomCalGridAdapter>();
	...
	/**
	 * customCalListener inform library client of the event happens inside
	 * Custom Calendar View
	 */
	private CustomCalListener customCalListener;

	public CustomCalListener getCustomCalListener() {
		return customCalListener;
	}
	 ...
	 */
	public CustomCalGridAdapter getNewDatesGridAdapter(int month, int year) {
		return new CustomCalGridAdapter(getActivity(), month, year,
				getCustomCalData(), extraData);
	}
	 ...
	 * @return
	 */
	public ArrayList<CustomCalGridAdapter> getDatePagerAdapters() {
		return datePagerAdapters;
	}

	/**
	 * customCalData return data belong to Custom Calendar View
	 *
	 * @return
	 */
	public HashMap<String, Object> getCustomCalData() {
		customCalData.clear();
		customCalData.put(SELECTED_DATES, selectedDates);
		customCalData.put(_MIN_DATE_TIME, minDateTime);
		customCalData.put(_MAX_DATE_TIME, maxDateTime);
		customCalData.put(START_DAY_OF_WEEK, Integer.valueOf(startDayOfWeek));
		customCalData.put(FIVE_WEEKS_IN_CALENDAR,
				Boolean.valueOf(fiveWeeksInCalendar));

		// For internal use
		customCalData.put(_BACKGROUND_FOR_DATETIME_MAP,
				backgroundForDateTimeMap);
		customCalData
				.put(_TEXT_COLOR_FOR_DATETIME_MAP, textColorForDateTimeMap);
	...
	/**
	 * Get current saved sates of the Custom Calendar View. Useful for handling
	 * rotation. It does not need to save state of SQUARE_TEXT_VIEW_CELL because
	 * this may change on orientation change
	 */
	public Bundle getSavedStates() {
		...
		bundle.putBoolean(FIVE_WEEKS_IN_CALENDAR, fiveWeeksInCalendar);
	...
	public void restoreStatesFromKey(Bundle savedInstanceState, String key) {
		if (savedInstanceState != null && savedInstanceState.containsKey(key)) {
			Bundle customCalSavedState = savedInstanceState.getBundle(key);
			setArguments(customCalSavedState);
		}
	}
	...
	public void restoreDialogStatesFromKey(FragmentManager manager,
		...
		CustomCalFragment existingDialog = (CustomCalFragment) manager
	...
	public void setCalendarDateTime(DateTime dateTime) {
		...
		// Notify listener
		if (customCalListener != null) {
			customCalListener.onChangeMonth(month, year);
		}
	 ...
	 * background by changing CustomCalFragment.selectedBackgroundDrawable, and
	 * change the text color CustomCalFragment.selectedTextColor before call
	 * this method. This method does not refresh view, need to call
	 * refreshView()
	 *
	 * @param fromDate
	 * @param toDate
	...
	public boolean isSixWeeksInCalendar() {
		return fiveWeeksInCalendar;
	}

	public void setSixWeeksInCalendar(boolean sixWeeksInCalendar) {
		this.fiveWeeksInCalendar = sixWeeksInCalendar;
	...
	/**
	 * Set custom calendar listener when user click on a date
	 *
	 * @param customCalListener
	 */
	public void setCustomCalListener(CustomCalListener customCalListener) {
		this.customCalListener = customCalListener;
	}
			...
			dateItemClickListener = new OnItemClickListener() {
				...
				public void onItemClick(AdapterView<?> parent, View view,
					...
					if (customCalListener != null) {
						...
						customCalListener.onSelectDate(date, view);
					}
				}
			};
		}
			...
			dateItemLongClickListener = new OnItemLongClickListener() {
				...
				public boolean onItemLongClick(AdapterView<?> parent,
					...
					if (customCalListener != null) {
						Date date = CalendarHelper
								.convertDateTimeToDate(dateTime);
						customCalListener.onLongClickDate(date, view);
					}
	...
	public void refreshView() {
		...
		for (CustomCalGridAdapter adapter : datePagerAdapters) {
			// Reset custom calendar data
			adapter.setCustomCalData(getCustomCalData());
	...
	protected void retrieveInitialArgs() {
		...
		if (args != null) {
			...
			// Get fiveWeeksInCalendar
			fiveWeeksInCalendar = args.getBoolean(FIVE_WEEKS_IN_CALENDAR, true);
	 ...
	 * @return
	 */
	public static CustomCalFragment newInstance(String dialogTitle, int month,
			int year) {
		CustomCalFragment f = new CustomCalFragment();
	...
	public View onCreateView(LayoutInflater inflater, ViewGroup container,
		...
		if (customCalListener != null) {
			customCalListener.onCustomCalViewCreated();
		}
	...
	private void setupDateGridPages(View view) {
		...
		// Current month
		CustomCalGridAdapter adapter0 = getNewDatesGridAdapter(
		...
		// when page change
		pageChangeListener.setCustomCalGridAdapters(datePagerAdapters);
		...
		dateViewPager.setSixWeeksInCalendar(fiveWeeksInCalendar);
		...
		for (int i = 0; i < NUMBER_OF_PAGES; i++) {
			...
			CustomCalGridAdapter adapter = datePagerAdapters.get(i);
	...
	public class DatePageChangeListener implements OnPageChangeListener {
		...
		private ArrayList<CustomCalGridAdapter> customCaldGridAdapters;
		 ...
		 * @return
		 */
		public ArrayList<CustomCalGridAdapter> getCustomCalGridAdapters() {
			return customCaldGridAdapters;
		}

		public void setCustomCalGridAdapters(
				ArrayList<CustomCalGridAdapter> customCaldGridAdapters) {
			this.customCaldGridAdapters = customCaldGridAdapters;
		}
		...
		private int getNext(int position) {
			return (position + 1) % CustomCalFragment.NUMBER_OF_PAGES;
		}
		...
		private int getPrevious(int position) {
			return (position + 3) % CustomCalFragment.NUMBER_OF_PAGES;
		}
		...
		public int getCurrent(int position) {
			return position % CustomCalFragment.NUMBER_OF_PAGES;
		}
		...
		public void refreshAdapters(int position) {
			...
			CustomCalGridAdapter currentAdapter = customCaldGridAdapters
					.get(getCurrent(position));
			CustomCalGridAdapter prevAdapter = customCaldGridAdapters
					.get(getPrevious(position));
			CustomCalGridAdapter nextAdapter = customCaldGridAdapters
		...
		public void onPageSelected(int position) {
			...
			CustomCalGridAdapter currentAdapter = customCaldGridAdapters
					.get(position % CustomCalFragment.NUMBER_OF_PAGES);
